...
class Dataset:
    def __init__(self, prev_shape , nodes, edges, i):
        self.QUERY_TRAIN, _, self.QUERY_TEST, self.QUERY_DOC, self.QUERY_DOC_TRUTH, \
         self.DOC_REPR, self.MAX_DCG , self.prev_shape = transform_all(prev_shape, nodes, edges, i)
...
def transform_all(prev_shape, nodes, edges, i):
    # train_data, test_data = import_all(dataset)
    train_data, test_data, prev_shape = generate_dataset(prev_shape, nodes, edges, i)
