...
def train(datadir,n_features,n_iterations,max_ep_len,save_model_freq,update_timestep,K_epochs,eps_clip,gamma,lr_actor,lr_critic,hidden_nodes,random_seed):
    ...
    i = 0
    nodes , edges = get_dataset()
    if(os.path.exists('latest_snapshot/nodes_snapshot.p')):
        with open('latest_snapshot/nodes_snapshot.p' , 'rb') as f: 
            node_seq = pickle.load(f)
        prev_shape = len(node_seq)
        # ppo_agent.load('Results/DBLP/DBLP-i_50-lr_ac_0.005,0.006-g_0.98-hnodes_32-max_T_30-up_T_100-epochs_3-seed_3_model')
        print('[INFO] Previous snapshot found and loaded, continuing training now')
        # data = Dataset(prev_shape, nodes, edges, i)
    print('Dataset loaded')
    while prev_shape < 20819006:
