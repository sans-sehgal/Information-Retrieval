...
class ActorCritic(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, hnodes):
        ...
        self.actor = nn.Sequential(
                        nn.LSTM(state_dim,64,1,batch_first=True),
                        extract_tensor(),
                        nn.Linear(64, hnodes),
                        nn.Tanh(),
    ...
    def act(self, state):
        ...
        action_probs = self.actor(state.unsqueeze(1))
    ...
    def evaluate(self, state, action):
        ...
        for s,a in zip(state,action):
            probs = self.actor(s.unsqueeze(1))
            ...
            state_value = self.critic(phi_s.unsqueeze(1))
...
class PPO:
    ...
    def select_action_test(self, state):
        ...
        with torch.no_grad():
            state = torch.FloatTensor(state) #.to(device)
            action_probs = self.policy.actor(state.unsqueeze(1))
            action_probs=action_probs.reshape(action_probs.shape[0])
            dist = Categorical(action_probs)
